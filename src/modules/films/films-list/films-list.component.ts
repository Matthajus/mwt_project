...
export class FilmsListComponent implements OnInit, AfterViewInit {
  ...
  columnsToDisplay = ['id', 'nazov', 'slovenskyNazov', 'rok', 'afi1998', 'afi2007', 'edit']
  @ViewChild(MatPaginator) paginator
    :
    MatPaginator
  @ViewChild(MatSort) sort: MatSort

  constructor(private filmsServerService: FilmsServerService, private snackBar: SnackbarService, public dialog: MatDialog) {
  }
  ...
  openDialog(film) {
    let dialogRef
    if (film){
      dialogRef = this.dialog.open(FilmEditComponent, {data: {film: film}});

    } else {
      dialogRef = this.dialog.open(FilmEditComponent);
    }

    dialogRef.afterClosed().subscribe(film => {
